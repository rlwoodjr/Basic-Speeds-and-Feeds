...
function updateValues(){
    ...
    if(  $('#selectUnits').val()=="inches"){
        BitDiameter =  parseFloat($('#selectBitDiameterinches').val());
        UnitConvert = 1;
    }else{
    ...
    // assign values for calculating ( Select material for the SFM and dd note)
   ...
   // double DFactor = Convert.ToDouble(TBoxFactor.Text);
    switch ($('#selectMachine').val())
    {
        ...
        case "E Series":
            ChipLoad = (0.007 * BitDiameter / UnitConvert + SFM / 10000000) * UnitConvert;
    ...
    FluteCount =  parseInt($('#selectFlutes').val());
    FeedRate = RPM * FluteCount * Chipload*UnitConvert;
    PlungeRate=0.5*FeedRate;
    Stepover=0.3*BitDiameter




    if (LabelRouterName == "Makita Dial:")  // Get Dial number on router based on RPM ( use liner equation y=mx+b)
    {
        if (RPM > 30000)
        {
            mDial = "6.0";
        ...
        {
            mDial = RPM * 0.00025 - 1.5;
            mRatio = "1.0";
            WMA[0] = T;
        }

    }
    else if (LabelRouterName== "DeWalt Dial:")
    {
        if (RPM > 27000)
        {
            dDial.Text = "6.0";
            dRatio.Text = Convert.ToString(RPM / 27000);
            dPM.Text = "27000";
            WMA[7] = T;
        }
        else if (RPM < 17000)
...
function updateNotes(noteNumber){
...
var note2="<p>There is a learning curve for cutting aluminum. The sweet spot for the cutting parameters is small and any deviation will "+
...
var note3 = "<p>The E Series has a fixed RPM. The feed rate and plunge rate have been adjusted for this RPM.</p>"

var note4 = "<p>Small bits can break easily. Reduce feed rate and depth of cut for longer bits with small diameters</p>"

var note5 = "<p>Large bits that are incorrectly set up can cause damage to the CNC and work piece. Reduce feed rate, stepover, and depth per pass.</p>"

var note6 = "<p>CNC routers spin at a high RPM.  One or two flutes are recommended for most materials.  An O-flute or single flute bit is recommended for cutting aluminum. </p>"

var note7 = "<p>Chip loads above 0.040\" (1mm) are not recommended.</p>"

var note8 = "<p>The spindle is not capable of spinning at this high of RPM.  The feed and plunge rate have been determined using the highest RPM for the "+
            "selected spindle.  A larger diameter bit or a bit with more flutes until the optimal RPM can be set is recommended.</p>"

var note9 = "<p>The spindle is not capable of spinning at this low of RPM.  The feed and plunge rate have been determined using the lowest RPM for the selected "+
...
if(noteNumber==1){$('#selectedNotes').html(note1)}
